...
public class CollinearTriplets {
	...
	private static long countCollinear(Point2d[] points) {
		...
		for (int i = 0; i < points.length; i++) {
			for (int j = i + 1; j < points.length; j++) {
				double slope = 0d, xIntercept, yIntercept; // Default slope
															// paralell to
															// y-axis
				if (points[i].x == points[j].x) {
					slope = Double.MAX_VALUE; // Horizontal slope parallel to
												// x-axis
	...
	/**
	 * Line which contains the starting point and slope so that you can identify
	 * exact line equals method is overridden to check whether any new line is
	 * coinciding or parallel
	 */
	static class Line {
		Point2d point;
		double slope;
